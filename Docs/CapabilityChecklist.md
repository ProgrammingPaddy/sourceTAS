...
dependencies), with the expansive detail in click-open drawers, and
**the function browser: every capability function, struct, and nested
method with its REAL source embedded** (clicking any function chip
opens the code; calls/called-by chips walk the intra-library call
graph). Update BOTH files every session; this file remains canonical.
**When library sources change, rerun `python
Tools/gen_capability_code.py`** — it re-extracts the code and call
graph and splices them into the dashboard's FNCODE block.
...
`TERMFNS` data (update both) — and, since session 43, embeds the
FULL SOURCE of every entry (plus every helper, struct, and nested
method, the `Fn::` export bodies, the `Move::` engine-mirror
implementations they forward to, and `MoveTick` itself) as the
function browser, generated by `Tools/gen_capability_code.py`.
...
- 2026-08-25a (session 43): **THE FUNCTION BROWSER — the dashboard
  ...
  viewable — "I want to be able to actually examine the code
  through that." Delivered: `Tools/gen_capability_code.py` extracts
  every function, struct, and nested method from the library
  sources (SolverCapability.h and SolverStrafe.h whole; the `Fn::`
  export bodies from SolverMove.cpp; the `Move::` engine-mirror
  implementations they forward to plus `MoveTick` itself;
  `Air::WishInputs`), computes the intra-library call graph, and
  splices it all into the dashboard — **183 entries (135 functions,
  8 methods, 40 structs), 6,240 source lines, 137 call edges**. The
  browser: a grouped, searchable list (names AND code text); a
  viewer with true file:line numbers, comment highlighting,
  registry-row chips, and calls / called-by chips that walk the
  graph (Fn::Duck → Move::Duck → the 68-line real body; MoveTick →
  its whole chain). Every function chip anywhere on the page now
  opens the code. Verified: all inventory chips resolve (0
  unresolved), all 183 captures end on their closing brace, search
  exercised. STANDING DUTY: rerun the generator whenever library
  sources change. No engine/suite code touched — no battery run
  needed.
